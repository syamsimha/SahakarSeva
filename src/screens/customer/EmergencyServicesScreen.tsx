...
export const EmergencyServicesScreen: React.FC<EmergencyServicesScreenProps> = ({
  ...
  const { user } = useAuth();
  const { t } = useLanguage();
  const { dispatchPriorityBooking, createBooking } = useBookings();
  const locationCtx = useLocation();
  const contextLocation = locationCtx?.currentLocation;
  const openLocationModal = locationCtx?.openLocationModal;
